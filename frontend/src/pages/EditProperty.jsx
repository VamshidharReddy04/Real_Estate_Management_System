...
export default function EditProperty() {
  ...
  useEffect(() => { dispatch(fetchProperty(id)); }, [id]);
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!form.title || !form.price || !form.features.area) return toast.error('Please fill all required fields');
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-8">
      <div className="flex items-center gap-3 mb-6">
        <button onClick={() => navigate(-1)} className="text-gray-500 hover:text-primary-600 transition-colors">← Back</button>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="card p-6 space-y-4">
          ...
          <div>
            ...
            <input value={form.title} onChange={e => set('title', e.target.value)} className={inputCls} maxLength={100} />
          </div>
          <div>
        ...
        <div className="card p-6 space-y-4">
          ...
          <div>
            ...
            <input value={form.location?.address || ''} onChange={e => set('location.address', e.target.value)} className={inputCls} />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={labelCls}>City</label>
              <input value={form.location?.city || ''} onChange={e => set('location.city', e.target.value)} className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>State</label>
              <input value={form.location?.state || ''} onChange={e => set('location.state', e.target.value)} className={inputCls} />
            </div>
          </div>
        </div>
        ...
        <div className="card p-6 space-y-4">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              ...
              <input type="number" value={form.features?.bedrooms || 0} min={0} onChange={e => set('features.bedrooms', e.target.value)} className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>Bathrooms</label>
              <input type="number" value={form.features?.bathrooms || 0} min={0} onChange={e => set('features.bathrooms', e.target.value)} className={inputCls} />
            </div>
            <div>
              <label className={labelCls}>Area * ({form.features?.areaUnit})</label>
              <input type="number" value={form.features?.area || ''} onChange={e => set('features.area', e.target.value)} className={inputCls} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className={labelCls}>Furnishing</label>
              <select value={form.features?.furnished || 'unfurnished'} onChange={e => set('features.furnished', e.target.value)} className={inputCls}>
                {FURNISHED_OPTIONS.map(o => <option key={o} value={o}>{o}</option>)}
              </select>
            </div>
            <div>
              <label className={labelCls}>Year Built</label>
              <input type="number" value={form.features?.yearBuilt || ''} onChange={e => set('features.yearBuilt', e.target.value)} className={inputCls} />
            </div>
          </div>
          <label className="flex items-center gap-2 cursor-pointer">
            <input type="checkbox" checked={form.features?.parking || false}
          ...
          <div className="card p-6">
            ...
            <div className="grid grid-cols-4 sm:grid-cols-6 gap-2">
              {property.images.map((img, i) => (
                <img key={i} src={img.url} alt="" className="w-full h-14 rounded-lg object-cover border" />
              ))}
            </div>
          </div>
        )}
        ...
        <div className="card p-6">
          ...
          {previews.length > 0 && (
            <div className="grid grid-cols-4 sm:grid-cols-6 gap-2 mt-3">
              {previews.map((src, i) => (
                <img key={i} src={src} alt="" className="w-full h-14 rounded-lg object-cover border border-primary-200" />
              ))}
            </div>
          )}
        </div>
